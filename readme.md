...
<table>
  <tbody>
    ...
    <tr>
      <td>
        We then convert each page's screenshot into text using macOS's built-in <strong>Vision OCR</strong>.
      </td>
      <td>
...
- _(This takes a few minutes to run)_
- This takes each of the page screenshots and runs them through macOS's built-in Vision OCR (via `src/ocr-macos.swift`, compiled on first run) to extract the raw text content from each page of the book. It runs locally and requires no API key; you need macOS with the Xcode command-line tools installed.
...
Transcription runs locally via macOS's Vision OCR, so there are **no LLM costs for the `image ⇒ text` step** (only the optional audiobook export costs anything). The screenshots are really good quality with no extra content, so you could swap in any other OCR solution quite easily.
